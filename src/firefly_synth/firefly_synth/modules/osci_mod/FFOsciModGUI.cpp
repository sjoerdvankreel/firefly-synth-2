...
FFMakeOsciModGUISectionMod(FBPlugGUI* plugGUI)
{
  ...
  grid->MarkSection({ { 0, 0 }, { 2, 3 } }, FBGridSectionMark::Border);
  grid->MarkSection({ { 0, 3 }, { 2, 6 } }, FBGridSectionMark::Alternate);
  grid->MarkSection({ { 0, 9 }, { 2, 9 } }, FBGridSectionMark::Border);
  return grid;
}
...
FFMakeOsciModTab(FBPlugGUI* plugGUI)
{
  ...
  return plugGUI->StoreComponent<FBModuleComponent>((int)FFModuleType::OsciMod, 0, section);
}