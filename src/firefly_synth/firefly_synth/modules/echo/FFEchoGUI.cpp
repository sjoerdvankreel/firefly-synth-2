...
MakeEchoTapsEditor(FBPlugGUI* plugGUI, bool global)
{
  ...
  grid->MarkSection({ { 0, 1 }, { 1, 1 } }, FBGridSectionMark::AlternateBackground);
  ...
  grid->MarkSection({ { 0, 2 }, { 1, 4 } }, FBGridSectionMark::AlternateBackground);
  ...
  grid->MarkSection({ { 0, 6 }, { 1, 3 } }, FBGridSectionMark::AlternateBackground);
  ...
  grid->MarkSection({ { 0, 9 }, { 1, 3 } }, FBGridSectionMark::AlternateBackground);
  ...
  for (int t = 0; t < FFEchoTapCount; t++)
  {
    ...
    if(t % 2 == 1)
      grid->MarkSection({ { t + 1, 0 }, { 1, 1 } }, FBGridSectionMark::AlternateBackground);
    ...
    if (t % 2 == 1)
      grid->MarkSection({ { t + 1, 1 }, { 1, 1 } }, FBGridSectionMark::AlternateBackground);
    ...
    if (t % 2 == 1)
      grid->MarkSection({ { t + 1, 2 }, { 1, 4 } }, FBGridSectionMark::AlternateBackground);
    ...
    if (t % 2 == 1)
      grid->MarkSection({ { t + 1, 6 }, { 1, 3 } }, FBGridSectionMark::AlternateBackground);
...
MakeEchoSectionFeedback(FBPlugGUI* plugGUI, bool global)
{
  auto topo = plugGUI->HostContext()->Topo();
  auto moduleType = global ? FFModuleType::GEcho : FFModuleType::VEcho;
  auto grid = plugGUI->StoreComponent<FBGridComponent>(plugGUI, true, std::vector<int> { 1, 1 }, std::vector<int> { 0, 0, 0, 1 });
  ...
  grid->Add(0, 3, plugGUI->StoreComponent<FBParamSlider>(plugGUI, mix, Slider::SliderStyle::LinearHorizontal));
  ...
  grid->Add(1, 3, plugGUI->StoreComponent<FBParamSlider>(plugGUI, amount, Slider::SliderStyle::LinearHorizontal));
  grid->MarkSection({ { 0, 0 }, { 1, 1 } }, FBGridSectionMark::DefaultBackground);
  return plugGUI->StoreComponent<FBCardComponent>(plugGUI, grid);
}
...
{
  FB_LOG_ENTRY_EXIT();
  auto select = plugGUI->StoreComponent<FBSelectComponent>(plugGUI, std::vector<int> { 1, 1 }, std::vector<int> { 1 });
