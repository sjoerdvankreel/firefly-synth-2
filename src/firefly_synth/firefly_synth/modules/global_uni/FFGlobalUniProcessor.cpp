...
FFGlobalUniProcessor::BeginVoice(int voice)
{
  ...
  for (int i = 0; i < (int)FFGlobalUniTarget::Count; i++)
  {
    if (_opType[i] == FFModulationOpType::Off)
      continue;
    if (_mode[i] != FFGlobalUniMode::AutoLinear && _mode[i] != FFGlobalUniMode::AutoExp)
      continue;
...
FFGlobalUniProcessor::BeginBlock(FBModuleProcState& state)
{
  ...
  for (int i = 0; i < (int)FFGlobalUniTarget::Count; i++)
  {
    ...
    if (_opType[i] == FFModulationOpType::Off || (_mode[i] != FFGlobalUniMode::AutoLinear && _mode[i] != FFGlobalUniMode::AutoExp))
      continue;
  ...
  bool graph, int voice, int voiceSlotInGroup, FBSArray<float, 16>& targetSignal)
{
  ...
  if (_mode[(int)targetParam] == FFGlobalUniMode::AutoLinear || _mode[(int)targetParam] == FFGlobalUniMode::AutoExp)
  {
