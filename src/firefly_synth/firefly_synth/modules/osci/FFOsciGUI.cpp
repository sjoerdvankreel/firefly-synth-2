...
{
  auto topo = plugGUI->HostContext()->Topo();
  std::vector<int> columnSizes = { 0, 0, 0, 0, 0, 1, 0, 0 };
  ...
  grid->Add(1, 1, plugGUI->StoreComponent<FBParamSlider>(plugGUI, dampKTrk, Slider::SliderStyle::RotaryVerticalDrag));
  ...
  grid->Add(1, 5, plugGUI->StoreComponent<FBParamSlider>(plugGUI, damp, Slider::SliderStyle::LinearHorizontal));

  auto feedbackKTrk = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringFeedbackKTrk, 0 } });
  grid->Add(2, 0, plugGUI->StoreComponent<FBParamLabel>(plugGUI, feedbackKTrk));
  grid->Add(2, 1, plugGUI->StoreComponent<FBParamSlider>(plugGUI, feedbackKTrk, Slider::SliderStyle::RotaryVerticalDrag));
  auto feedback = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringFeedback, 0 } });
  grid->Add(2, 4, plugGUI->StoreComponent<FBParamLabel>(plugGUI, feedback));
  grid->Add(2, 5, plugGUI->StoreComponent<FBParamSlider>(plugGUI, feedback, Slider::SliderStyle::LinearHorizontal));

  auto lpKTrk = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringLPKTrk, 0 } });
  grid->Add(3, 0, plugGUI->StoreComponent<FBParamLabel>(plugGUI, lpKTrk));
  grid->Add(3, 1, plugGUI->StoreComponent<FBParamSlider>(plugGUI, lpKTrk, Slider::SliderStyle::RotaryVerticalDrag));
  auto lpOn = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringLPOn, 0 } });
  grid->Add(3, 2, plugGUI->StoreComponent<FBParamLabel>(plugGUI, lpOn));
  grid->Add(3, 3, plugGUI->StoreComponent<FBParamToggleButton>(plugGUI, lpOn));
  auto lpFreq = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringLPFreq, 0 } });
  grid->Add(3, 4, plugGUI->StoreComponent<FBParamLabel>(plugGUI, lpFreq));
  grid->Add(3, 5, plugGUI->StoreComponent<FBParamSlider>(plugGUI, lpFreq, Slider::SliderStyle::LinearHorizontal));
  auto lpRes = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringLPRes, 0 } });
  grid->Add(3, 6, plugGUI->StoreComponent<FBParamLabel>(plugGUI, lpRes));
  grid->Add(3, 7, plugGUI->StoreComponent<FBParamSlider>(plugGUI, lpRes, Slider::SliderStyle::RotaryVerticalDrag));

  auto hpKTrk = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringHPKTrk, 0 } });
  grid->Add(4, 0, plugGUI->StoreComponent<FBParamLabel>(plugGUI, hpKTrk));
  grid->Add(4, 1, plugGUI->StoreComponent<FBParamSlider>(plugGUI, hpKTrk, Slider::SliderStyle::RotaryVerticalDrag));
  auto hpOn = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringHPOn, 0 } });
  grid->Add(4, 2, plugGUI->StoreComponent<FBParamLabel>(plugGUI, hpOn));
  grid->Add(4, 3, plugGUI->StoreComponent<FBParamToggleButton>(plugGUI, hpOn));
  auto hpFreq = topo->audio.ParamAtTopo({ { (int)FFModuleType::Osci, moduleSlot }, { (int)FFOsciParam::StringHPFreq, 0 } });
  grid->Add(4, 4, plugGUI->StoreComponent<FBParamLabel>(plugGUI, hpFreq));
  grid->Add(4, 5, plugGUI->StoreComponent<FBParamSlider>(plugGUI, hpFreq, Slider::SliderStyle::LinearHorizontal));
