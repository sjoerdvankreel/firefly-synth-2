...
MakeLFOSectionBlock(FBPlugGUI* plugGUI, FFModuleType moduleType, int moduleSlot, int block)
{
  ...
  grid->Add(1, 0, plugGUI->StoreComponent<FBParamComboBox>(plugGUI, opType));
  auto waveMode = topo->audio.ParamAtTopo({ { (int)moduleType, moduleSlot }, { (int)FFLFOParam::WaveMode, block } });
  grid->Add(0, 1, plugGUI->StoreComponent<FBParamLabel>(plugGUI, waveMode));
  grid->Add(0, 2, plugGUI->StoreComponent<FBParamComboBox>(plugGUI, waveMode));
  auto rateHz = topo->audio.ParamAtTopo({ { (int)moduleType, moduleSlot }, { (int)FFLFOParam::RateHz, block } });
  grid->Add(1, 1, plugGUI->StoreComponent<FBParamLabel>(plugGUI, rateHz));
  grid->Add(1, 2, plugGUI->StoreComponent<FBParamSlider>(plugGUI, rateHz, Slider::SliderStyle::LinearHorizontal));
  auto rateBars = topo->audio.ParamAtTopo({ { (int)moduleType, moduleSlot }, { (int)FFLFOParam::RateBars, block } });
  grid->Add(1, 1, plugGUI->StoreComponent<FBParamLabel>(plugGUI, rateBars));
...
FFMakeLFOGUI(FBPlugGUI* plugGUI)
{
  FB_LOG_ENTRY_EXIT();
  auto topo = plugGUI->HostContext()->Topo();
  auto tabParam = topo->gui.ParamAtTopo({ { (int)FFModuleType::Settings, 0 }, { (int)FFSettingsGUIParam::LFOSelectedTab, 0 } });
