...
MakeSettingsTab(FBPlugGUI* plugGUI)
{
  ...
  grid->Add(1, 3, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, hilightTweak));
  grid->Add(1, 4, plugGUI->StoreComponent<FBGUIParamToggleButton>(plugGUI, hilightTweak));
  auto knobVisualsMode = topo->gui.ParamAtTopo({ { (int)FFModuleType::Settings, 0 }, { (int)FFSettingsGUIParam::KnobVisualsMode, 0 } });
  grid->Add(0, 5, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, knobVisualsMode));
  grid->Add(0, 6, plugGUI->StoreComponent<FBGUIParamComboBox>(plugGUI, knobVisualsMode));
  auto graphVisualsMode = topo->gui.ParamAtTopo({ { (int)FFModuleType::Settings, 0 }, { (int)FFSettingsGUIParam::GraphVisualsMode, 0 } });
  grid->Add(1, 5, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, graphVisualsMode));
  grid->Add(1, 6, plugGUI->StoreComponent<FBGUIParamComboBox>(plugGUI, graphVisualsMode));
