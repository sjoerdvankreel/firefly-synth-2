...
MakeMasterSectionMain(FBPlugGUI* plugGUI)
{
  ...
  auto receiveNotes = topo->audio.ParamAtTopo({ { (int)FFModuleType::Master, 0 }, { (int)FFMasterParam::ReceiveNotes, 0 } });
  grid->Add(1, 4, plugGUI->StoreComponent<FBParamLabel>(plugGUI, receiveNotes));
  grid->Add(1, 5, plugGUI->StoreComponent<FBParamToggleButton>(plugGUI, receiveNotes));
