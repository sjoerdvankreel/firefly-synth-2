...
FFMakeGUISettingsGUI(FBPlugGUI* plugGUI)
{
  ...
  grid->Add(0, 2, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, showMatrix));
  ...
  grid->Add(0, 3, showMatrixToggle);
  auto graphRenderMode = topo->gui.ParamAtTopo({ { (int)FFModuleType::GUISettings, 0 }, { (int)FFGUISettingsGUIParam::GraphRenderMode, 0 } });
  grid->Add(0, 4, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, graphRenderMode));
  grid->Add(0, 5, plugGUI->StoreComponent<FBGUIParamComboBox>(plugGUI, graphRenderMode));
