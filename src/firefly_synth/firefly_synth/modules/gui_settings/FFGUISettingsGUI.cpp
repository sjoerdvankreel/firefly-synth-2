...
FFMakeGUISettingsGUI(FBPlugGUI* plugGUI)
{
  ...
  grid->Add(1, 0, plugGUI->StoreComponent<FBAutoSizeLabel>(plugGUI, "Theme"));
  PopupMenu themeMenu = {};
  ...
  grid->Add(1, 1, themeCombo);   

  auto hilightMod = topo->gui.ParamAtTopo({ { (int)FFModuleType::GUISettings, 0 }, { (int)FFGUISettingsGUIParam::HilightMod, 0 } });
  grid->Add(2, 0, plugGUI->StoreComponent<FBGUIParamLabel>(plugGUI, hilightMod));
  grid->Add(2, 1, plugGUI->StoreComponent<FBGUIParamToggleButton>(plugGUI, hilightMod));
