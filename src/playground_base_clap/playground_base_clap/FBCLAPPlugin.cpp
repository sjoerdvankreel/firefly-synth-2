...
{
  auto iter = _topo->audioParamTagToIndex.find(paramId);
  return iter == _topo->audioParamTagToIndex.end() ? -1 : iter->second;
}
...
{
  auto iter = _topo->audioParamTagToIndex.find(paramId);
  if (iter == _topo->audioParamTagToIndex.end())
    return false;
...
FBCLAPPlugin::isValidParamId(
  clap_id paramId) const noexcept
{
  return _topo->audioParamTagToIndex.find(paramId) != _topo->audioParamTagToIndex.end();
}
  ...
  const clap_output_events* out, bool pushToProcBlock)
{
  ...
  while (_mainToAudioEvents.try_dequeue(uiEvent))
  {
    ...
    auto const& param = _topo->audioParams[uiEvent.paramIndex];
    switch (uiEvent.type)
    {
  ...
  const clap_process* process) noexcept 
{
  ...
  for (int i = 0; i < inEventCount; i++)
  {
    ...
    case CLAP_EVENT_PARAM_MOD:
      ...
      if ((iter = _topo->audioParamTagToIndex.find(modFromHost->param_id)) != _topo->audioParamTagToIndex.end())
    ...
    case CLAP_EVENT_PARAM_VALUE:
      ...
      if ((iter = _topo->audioParamTagToIndex.find(valueFromHost->param_id)) != _topo->audioParamTagToIndex.end())
      {
        normalized = FBCLAPToNormalized(_topo->audioParams[iter->second].static_, valueFromHost->value);
