...
MakeSectionAll(FBPlugGUI* plugGUI, int moduleSlot)
{
  ...
  auto type = topo->audio.ParamAtTopo({ (int)FFModuleType::VEffect, moduleSlot, (int)FFEffectParam::Type, 0 });
  ...
  auto shaperGain = topo->audio.ParamAtTopo({ (int)FFModuleType::VEffect, moduleSlot, (int)FFEffectParam::ShaperGain, 0 });
...
{
  return plugGUI->StoreComponent<FBModuleTabComponent>(plugGUI, (int)FFModuleType::VEffect, TabFactory);
}