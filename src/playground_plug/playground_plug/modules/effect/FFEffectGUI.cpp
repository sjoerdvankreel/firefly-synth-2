...
MakeSectionBlock(FBPlugGUI* plugGUI, int moduleSlot, int block)
{
  ...
  auto shapeMode = topo->audio.ParamAtTopo({ (int)FFModuleType::Effect, moduleSlot, (int)FFEffectParam::ShapeMode, block });
  grid->Add(1, 0, plugGUI->StoreComponent<FBParamLabel>(plugGUI, shapeMode));
  grid->Add(1, 1, plugGUI->StoreComponent<FBParamComboBox>(plugGUI, shapeMode));
  ...
  grid->Add(1, 2, plugGUI->StoreComponent<FBParamLabel>(plugGUI, distAmt));
  grid->Add(1, 3, plugGUI->StoreComponent<FBParamSlider>(plugGUI, distAmt, Slider::SliderStyle::RotaryVerticalDrag));
  auto distFdbk = topo->audio.ParamAtTopo({ (int)FFModuleType::Effect, moduleSlot, (int)FFEffectParam::DistFdbk, block });
  grid->Add(1, 4, plugGUI->StoreComponent<FBParamLabel>(plugGUI, distFdbk));
  grid->Add(1, 5, plugGUI->StoreComponent<FBParamSlider>(plugGUI, distFdbk, Slider::SliderStyle::RotaryVerticalDrag));
