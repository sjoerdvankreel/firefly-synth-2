...
MakeSectionAll(FBPlugGUI* plugGUI, int moduleSlot)
{
  ...
  auto voices = topo->ParamAtTopo({ (int)FFModuleType::Master, moduleSlot, (int)FFMasterParam::Voices, 0 });
  ...
  auto hostSmoothTime = topo->ParamAtTopo({ (int)FFModuleType::Master, moduleSlot, (int)FFMasterParam::HostSmoothTime, 0 });
