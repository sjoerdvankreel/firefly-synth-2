...
MakeSectionAll(FBPlugGUI* plugGUI, int moduleSlot)
{
  ...
  for (int i = 0; i < FFOsciModSlotCount; i++)
  {
    grid->Add(0, 1 + i * 3, 2, 1, plugGUI->StoreComponent<FBAutoSizeLabel>(FFOsciModFormatSlot(i)));
    ...
    grid->Add(1, 1 + i * 3 + 1, plugGUI->StoreComponent<FBParamComboBox>(plugGUI, fmMode));
