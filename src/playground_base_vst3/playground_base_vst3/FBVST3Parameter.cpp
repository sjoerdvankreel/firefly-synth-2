...
{
  auto text = _topo->NonRealTime().NormalizedToText(FBValueTextDisplay::Text, valueNormalized);
...
FBVST3Parameter::fromString(const TChar* string, ParamValue& valueNormalized) const
{
  ...
  if (!FBVST3CopyFromString128(string, str))
    return false;
  auto parsed = _topo->NonRealTime().TextToNormalized(FBValueTextDisplay::Text, str);
  if (!parsed.has_value())
    return false;
