...
_tabs(TabbedButtonBar::Orientation::TabsAtTop)
{
  ...
  if(module.slotCount == 1)
    _tabs.addTab(module.name, Colours::black, 
      tabFactory(topo, plugGUI, hostContext, 0), false);
  else
  {
    _tabs.addTab(module.name, Colours::black, nullptr, false);
    for (int i = 0; i < module.slotCount; i++)
      _tabs.addTab(std::to_string(i + 1), Colours::black, 
        tabFactory(topo, plugGUI, hostContext, i), false);
